...
class Solution {
    ...
    function firstBadVersion($n) {
        ...
        while ($start < $finish) {  // Исправлено условие
            $middle = $start + floor(($finish - $start) / 2);  // Добавлен floor()
            if ($this->isBadVersion($middle)) {  // Добавлен $this->
                $finish = $middle;
            } else {
                $start = $middle + 1;  // Исправлено: +1
            }
        }

        return $start;  // Упрощено возвращение
    }

    // Временная реализация для тестирования
    function isBadVersion($n) {
        static $badVersion = 4;  // Первая плохая версия
        return $n >= $badVersion;  // Все версии после плохой тоже плохие
    }
}

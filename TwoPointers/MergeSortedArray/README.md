...
*You are giver two integer arrays `nums1` and 
`nums2`, sorted in **non-decreasing order**,
and two integers `m` and `n`, representing the
number of elements in `nums1` and `nums2` 
respectively.*

***Merge** `nums1` and `nums2` into single array
sorted in **non-decreasing order***

*The final sorted array should not be returned
by the function, but instead be stored inside
the array `nums1`. To accomodate this, `nums1`
has a length of `m+n`, where the first `m` 
elements denote the elements that should be 
merged, and the last `n` elements are set to `0`
...
and [2,5,6]. The result of the merge is [1,2,2,3,5,6]
with underlined elements coming from nums1.
```
...
Note that because m = 0, there no elements in 
